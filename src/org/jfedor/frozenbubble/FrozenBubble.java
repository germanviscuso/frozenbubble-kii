...
public class FrozenBubble extends Activity implements GameView.GameListener,
	...
	private final int[] MODlist = { R.raw.ambientpower, R.raw.ambientlight,
			R.raw.androidrupture, R.raw.artificial, R.raw.aftertherain,
			R.raw.bluestars, R.raw.chungababe, R.raw.crystalhammer,
			R.raw.dreamscope, R.raw.freefall, R.raw.gaeasawakening,
			R.raw.homesick, R.raw.ifcrystals, R.raw.popcorn,
			R.raw.stardustmemories, R.raw.sunshineofthemorningsun,
	 ...
	 * 
	 * This method is called when the activity is started. The activity may have
	 * been reconfigured or the system may have killed the process, after which
	 * it regained focus to invoke this method.
	 */
	@Override
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case MENU_DONT_RUSH_ME:
			setDontRushMe(true);
			prefs.set("dontRushMe", dontRushMe);
			break;
		case MENU_RUSH_ME:
			setDontRushMe(false);
			prefs.set("dontRushMe", dontRushMe);
	...
	protected void onPause() {
		...
		if ((null == i) || !activityCustomStarted) {
			prefs.set("level", mGameThread.getCurrentLevelIndex());
		} else {
			// Editor's intent is running.
			prefs.set("levelCustom", mGameThread.getCurrentLevelIndex());
		}
		try {
			prefs.save();
		} catch (Exception e) {
	...
	/**
	 * Invoked when the Activity is finishing or being destroyed by the system.
	 */
	@Override
	...
	private void restoreGamePrefs() {
		dontRushMe = prefs.getBoolean("dontRushMe", false);
		fullscreen = prefs.getBoolean("fullscreen", true);
		gameMode = prefs.getInt("gameMode", GAME_NORMAL);
		musicOn = prefs.getBoolean("musicOn", true);
		soundOn = prefs.getBoolean("soundOn", true);
		targetMode = prefs.getInt("targetMode", POINT_TO_SHOOT);
	 ...
	 * 
	 * <p>
	 * If the level isn't specified from the editor, then the player selected
	 * the option to continue playing from the last level played, so use the
	 * last level played instead.
	 * 
	 * @param intent
	...
	private void soundOptionsDialog() {
		...
		//
		//
				.setMultiChoiceItems(R.array.sound_options_array,
						isCheckedItem,
						new DialogInterface.OnMultiChoiceClickListener() {
							...
							public void onClick(DialogInterface dialog, int which, boolean isChecked) {
								...
								case 1:
									...
									prefs.set("musicOn", musicOn);
									break;
								}
								try {
									prefs.save();
								} catch (Exception e) {
									Toast.makeText((Context) dialog, "Kii Cloud connection error", Toast.LENGTH_SHORT).show();
									finish();
								}
							}
						})
	...
	private void targetOptionsDialog() {
		...
		//
				.setSingleChoiceItems(R.array.shoot_mode_array, targetMode,
						new DialogInterface.OnClickListener() {
							@Override
							public void onClick(DialogInterface builder,
									int which) {
								switch (which) {
								case 0:
				...
				.setPositiveButton(R.string.ok,
						new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface builder, int id) {
								// User clicked OK.
								prefs.set("targetMode", targetMode);
								try {
									prefs.save();
								} catch (Exception e) {
									Toast.makeText((Context) builder, "Kii Cloud connection error", Toast.LENGTH_SHORT).show();
									finish();
								}
							}
						});
	...
	public synchronized void setTargetMode(int tm) {
		...
		if ((targetMode == ROTATE_TO_SHOOT)
			...
			// APIs, but a definition was not yet explicitly defined.
			//
	 ...
	 * 
	 * @param startPaused
	 *            - If false, the song starts playing immediately. Otherwise it
	 *            is paused and must be unpaused to start playing.
	 */
	private void newMusicPlayer(boolean startPaused) {
		// Create a new music player.
		resplayer = new MODResourcePlayer(this);
		// Ascertain which song to play.
		int modNow = mGameView.getThread().getCurrentLevelIndex()
				% MODlist.length;
		// Load the mod file.
	 ...
	 * 
	 * @param startPlaying
	 *            - If true, the song starts playing immediately. Otherwise it
	 *            is paused and must be unpaused to start playing.
	 */
	...
	/**
	 * This function determines whether a music player instance needs to be
	 * created or if one already exists. Then, based on the current level, the
	 * song to play is calculated and loaded. If desired, the song will start
	 * playing immediately, or it can remain paused.
	 * 
	 * @param startPlaying
	 *            - If true, the song starts playing immediately. Otherwise it
	 *            is paused and must be unpaused to start playing.
